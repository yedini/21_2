...
theta <- seq(-100, 100, length.out=50000)
...
data.frame(theta, s) %>% ggplot(aes(theta, s)) +
  ...
  geom_vline(xintercept=maxx, color="seagreen3", size=1, linetype=2)+
  annotate("text", x=maxx+10, y=-170, label=paste0("x=", round(maxx, 4)))
```
             
             
```{r}
#### NR method for problem 2.1 ####
rf_by_nr_2 <- function(func, x, thres=10^(-10), maxiter=1000) {
  error <- 1        # stopping rule: abs conv. criterion
  niter <- 0      # number of iteration
...
for (s in sp) {
  ...
  tryCatch(print(rf_by_nr_2(function(theta) loglik(theta, sample), s)),
           error = function(e) print("Cannot find MLE"))
}
```
         
```{r}
rf_by_nr_2(function(theta) loglik(theta, sample), mean(sample))
```
...
#### d.
```{r}
#### secant method for problem 2.1
rf_by_se_2 <- function(func, x_1, x, thres=10^(-10), maxiter=1000) {
  error <- 1        # stopping rule: abs conv. criterion
  niter <- 0      # number of iteration
...
```
```{r}
rf_by_se_2(function(theta) loglik(theta, sample), -2, -1)
```
```{r}
rf_by_se_2(function(theta) loglik(theta, sample), -3, 3)
```
```{r}
rf_by_se_2(function(theta) loglik(theta, sample), 0, 20)
```
...
```{r}
start_time <- Sys.time()
rf_by_nr_2(function(theta) loglik(theta, sample), -1)
end_time <- Sys.time()
print(paste("NR method speed:", end_time-start_time))
```
```{r}
start_time <- Sys.time()
rf_by_bisec(function(theta) loglik(theta, sample), -1, 1)
end_time <- Sys.time()
print(paste("bisection method speed:", end_time-start_time))
```
```{r}
start_time <- Sys.time()
rf_by_se_2(function(theta) loglik(theta, sample), -2, -1)
end_time <- Sys.time()
print(paste("secant method speed:", end_time-start_time))
```
          
속도: secant < bisection < NR
안정성: ??         
...
theta <- seq(0, 30, length.out=1000)
...
```{r}
# start_time <- Sys.time()
# rf_by_nr_2(function(theta) loglik2(theta, sample), 20)
# end_time <- Sys.time()
# print(paste("NR method speed:", end_time-start_time))
# ```
# ```{r}
# start_time <- Sys.time()
# rf_by_bisec(function(theta) loglik2(theta, sample), -1, 1)
# end_time <- Sys.time()
# print(paste("bisection method speed:", end_time-start_time))
# ```
# ```{r}
# start_time <- Sys.time()
# rf_by_se_2(function(theta) loglik2(theta, sample), -2, -1)
# end_time <- Sys.time()
# print(paste("secant method speed:", end_time-start_time))
```
...
theta <- seq(-pi, pi, length.out=5000)
...
#### c.
```{r}
# starting point: -0.058 (MMEof theta)
rf_by_nr_2(function(theta) loglik_2_2(theta, sample), -0.058)
```
          
```{r}
# starting point: -2.7
rf_by_nr_2(function(theta) loglik_2_2(theta, sample), -2.7)
```
...
for (i in 1:length(sv)) {
  result[i] <- rf_by_nr_2(function(theta) loglik_2_2(theta, sample), sv[i])
}
...
change <- data.frame(sv_front=c(), sol_front=c(), sv_back=c(), sol_back=c())
for (i in 1:(nrow(data)-1)) {
  if (data$result[i] != data$result[i+1]) {
    change <- rbind(change, data.frame(sv_front=data$sv[i], sol_front=data$result[i], sv_back=data$sv[i+1], sol_back=data$result[i+1])) 
  }
}
change
```
```{r}
change$diff <- change$sol_front - change$sol_back
change[which.min(abs(change$diff)),]
```
           
: starting value가 2.478543에서 2.510117로 바뀌었을 때 NR method의 solution이 2.47537에서 2.51359로 바뀐다. solution이 바뀌는 지점들 중 두 solution의 차가 가장 작은 지점이다.
